    ...
    addToEvent() {
      this.eventsSelected.forEach((event) => {
        axios
          ...
          .then(() => { this.getEventsRegistered(); alert('Client Added') })
          .catch((error) => {
      ...
      <form @submit.prevent="handleSubmitForm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
          ...
          <div class="flex flex-col">
            <label class="block">
              ...
              <span class="text-black" v-if="v$.client.firstName.$error">
                <p class="text-red-700" v-for="error of v$.client.firstName.$errors" :key="error.$uid">
                  {{ error.$message }}!
                </p>
              </span>
            </label>
          ...
          <div class="flex flex-col">
            <label class="block">
              ...
              <span class="text-black" v-if="v$.client.lastName.$error">
                <p class="text-red-700" v-for="error of v$.client.lastName.$errors" :key="error.$uid">
                  {{ error.$message }}!
                </p>
              </span>
            </label>
          ...
          <div class="flex flex-col">
            <label class="block">
              ...
              <span class="text-black" v-if="v$.client.email.$error">
                <p class="text-red-700" v-for="error of v$.client.email.$errors" :key="error.$uid">
                  {{ error.$message }}!
                </p>
              </span>
            </label>
          ...
          <div class="flex flex-col">
            <label class="block">
              ...
              <span class="text-black" v-if="v$.client.phoneNumber.primary.$error">
                <p class="text-red-700" v-for="error of v$.client.phoneNumber.primary.$errors" :key="error.$uid">
                  {{ error.$message }}!
                </p>
              </span>
            </label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
          <div class="flex justify-between mt-10 mr-20">
            <button @click="updateClient" type="submit" class="bg-green-700 text-white rounded">
              Update Client
            </button>
          </div>
          <div class="flex justify-between mt-10 mr-20">
            <button @click="deregisterClient" type="submit" class="bg-red-700 text-white rounded">
              Delete Client
            </button>
          </div>
          <div class="flex justify-between mt-10 mr-20">
        ...
        <div class="mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
          ...
          <div class="flex flex-col col-span-2">
            <table class="min-w-full shadow-md rounded">
              ...
              <tbody class="divide-y divide-gray-300">
                ...
                <tr @click="editEvent(event._id)" v-for="event in eventsRegistered" :key="event._id">
          ...
          <div class="flex flex-col">
            ...
            <VueMultiselect
              class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
              v-model="eventsSelected" :options="eventsAll" :custom-label="nameWithDate" :multiple="true"
              :close-on-select="true" placeholder="Select Events to be added" label="date" track-by="name" />
            <div class="flex justify-between">
              <button @click="addToEvent" type="submit" class="mt-5 bg-red-700 text-white rounded">
