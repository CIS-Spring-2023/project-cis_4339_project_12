...
export default {
  ...
  created() {
    ...
    //fetch services that are set to active
    axios.get(`${apiURL}/services/search?val=true&searchBy=status`).then((res) => {
      ...
      this.services.forEach((e) => {
        axios.get(`${apiURL}/services/id/${e._id}`).then((res) => {
          this.AvailableServices.push(res.data)
        })
      })
    })
  },
  methods: {
    ...
    handleEventUpdate() {
      //update the event using the the stored ID and event variabels
      this.alertMessage = "Event updated";
    ...
    editClient(clientID) {
      //push edits that are specified in the arguments
      this.alertMessage = "Client updated";
      this.$router.push({ name: 'updateclient', params: { id: clientID } })
    },
    eventDelete() {
      this.alertMessage = "Event deleted";
      //delete the event associated with this ID
  ...
  <main>
    ...
    <div class="px-10 py-20">
      <form @submit.prevent="handleSubmitForm">
          ...
          <div class="flex flex-col grid-cols-3">
            ...
            <div v-for="service in AvailableServices">
              <label :for="service" class="inline-flex items-center">
                <input type="checkbox" :id="service.name.replaceAll(' ', '')" :value="service.name"
        ...
        <!-- grid container -->
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10" v-if="user.isEditor">
